...
3. Diffs the fetched batch against `content_url`s already in the table, and only for the genuinely new ones: translates title + summary (see "Translation" below), then upserts everything keyed on `content_url` with `ignoreDuplicates`.
