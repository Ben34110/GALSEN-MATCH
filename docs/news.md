...
The notification also carries the first new article's cover image (whichever one in the batch actually has one — some sources' images get nulled by the reachability check) as the Notification API's `image` field, distinct from `icon` (the small app badge). On iOS 16.4+ and Android, this is what shows up when you press-and-hold / expand the notification — not visible in the collapsed banner, only once expanded.
